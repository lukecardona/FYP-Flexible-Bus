...
class Request():
    ...
    def _getTimeBetweenPoints(self, first, second):
        firstCord = str(first.getLongitude()) + "," + str(first.getLatitude())
        secondCord =  str(second.getLongitude()) + "," + str(second.getLatitude())
        ...
        if response.status_code == 200:
            data = response.json()
            return data['routes'][0]['duration']  # Duration in secondsq
        else:
            Exception("Failed to get a response from the OSRM server")
    
    def _getTimeWindows(self, origin, destination, time):
        originTime = time + timedelta(seconds=TIMEWINDOW)
        destinationTime = time + timedelta(self._getTimeBetweenPoints(origin, destination)) + timedelta(seconds=TIMEWINDOW)
...
class Route:
    ...
    def calculateTotalDeadHeadDistance(self, vehiclePosition, startingCapacity):
        if self.routeList.getSize() == 0:
            return 0
        deadHeadingDistance = 0
        data = self.getRequestOSRMJSON(vehiclePosition)
        listOfCords = self.routeList.getListOfCords()
        currentCapacity = startingCapacity
        for i in range(len(data['routes'][0]['legs'])):
            if currentCapacity == 0:
                deadHeadingDistance += data['routes'][0]['legs'][i]['distance']
            
            if listOfCords[i].getStart() == True:
                currentCapacity += listOfCords[i].getPassengerAmount()
            else:
                currentCapacity -= listOfCords[i].getPassengerAmount()
        return deadHeadingDistance
...
class Vehicle():
    ...
    def getDeadHeadDistance(self):
        return self.route.calculateTotalDeadHeadDistance(self.currentPosition,self.currentCapacity)
